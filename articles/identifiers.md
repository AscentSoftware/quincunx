...
The data provided by quincunx includes various identifiers. This
document serves as a quick reference list for the identifiers you might
find in objects returned by quincunx functions.
...
other special meaning outside the tables they appear (`sample_id`,
`effect_size_id`, `classification_metrics_id` and `other_metrics_id`.)
...
are provided by a set of functions of the form `<from_id>_to_<to_id>()`,
e.g., the function
...
As a technical side note, those mappings marked with ↯ are achieved by
successive requests to different endpoints, e.g., for the function
[`pss_to_pgs()`](https://ascentsoftware.github.io/quincunx/reference/pss_to_pgs.md)
to find the associated `pgs_id` with one or more `pss_id`, it needs
first to map `pss_id` to `ppm_id`, and only then from those obtained
`ppm_id` to `pgs_id`.
